...
class DeclarationGenerator(
    ...
    private val hierarchyDisjointUnions: DisjointUnions<IrClass>,
    ...
    private val classVtableTypeDeclarations = mutableMapOf<List<IrClass>, WasmStructDeclaration>()
    private fun createDeclarationByInterfaceList(iFacesUnion: List<IrClass>) {
        classVtableTypeDeclarations.getOrPut(iFacesUnion) {
            val fields = iFacesUnion.mapIndexed { index, iFace ->
                context.defineClassITableInterfaceSlot(iFace.symbol, index)
                WasmStructFieldDeclaration(
                    name = "${iFace.fqNameWhenAvailable.toString()}.itable",
                    type = WasmRefNullType(WasmHeapType.Type(context.referenceVTableGcType(iFace.symbol))),
                    isMutable = false
                )
            }
            WasmStructDeclaration(
                ...
                iFacesUnion.forEach {
                    context.defineClassITableGcTypeForInterface(it.symbol, struct)
                }
            }
        }
    }

    private fun createVirtualTableStruct(methods: List<VirtualMethodMetadata>, name: String): WasmStructDeclaration {
    ...
    private fun createVTable(metadata: ClassMetadata) {
        ...
        val vtableStruct = createVirtualTableStruct(metadata.virtualMethods, vtableName)
        context.defineVTableGcType(metadata.klass.symbol, vtableStruct)

        if (klass.modality == Modality.ABSTRACT) return
        ...
        val initVTableGlobal = buildWasmExpression {
            metadata.virtualMethods.forEachIndexed { i, method ->
                if (method.function.modality != Modality.ABSTRACT) {
                    ...
                    buildRefNull(vtableStruct.fields[i].type.getHeapType()) //This erased by DCE so abstract version appeared in non-abstract class
                }
            }
    ...
    private fun createClassITable(metadata: ClassMetadata) {
        val klass = metadata.klass
        if (klass.modality == Modality.ABSTRACT) return
        val supportedInterface = metadata.interfaces.firstOrNull() ?: return

        val classInterfaceType = context.referenceClassITableGcTypeForInterface(supportedInterface.symbol)
        val interfaceList = hierarchyDisjointUnions.getUnionByElement(supportedInterface)
        createDeclarationByInterfaceList(interfaceList)

        val initITableGlobal = buildWasmExpression {
            for (iFace in interfaceList) {
                val iFaceSymbol = iFace.symbol
                val iFaceVTableGcType = context.referenceVTableGcType(iFaceSymbol)
                val iFaceVTableGcNullHeapType = WasmHeapType.Type(iFaceVTableGcType)

                if (!metadata.interfaces.contains(iFace)) {
                    buildRefNull(iFaceVTableGcNullHeapType)
                    continue
                }

                for (method in context.getInterfaceMetadata(iFaceSymbol).methods) {
    ...
    override fun visitClass(declaration: IrClass) {
        ...
        if (declaration.isInterface) {
            if (hierarchyDisjointUnions.hasUnionWithElement(declaration)) {
